...
public class TasksController {
    ...
    @Operation(summary = "Update a task",
            description = "Update a task with the given id, title, due date and status",
            tags = {"Tasks", "Update"})
    @ApiResponses({
            @ApiResponse(responseCode = "200", content = {@Content(schema = @Schema(implementation = TasksController.class), mediaType = "application/json")}),
            @ApiResponse(responseCode = "404", content = {@Content(schema = @Schema())}),
    ...
    public ResponseEntity<Task> updateTask(@Parameter(description = "Update a task by adding required info") @RequestBody Task Task,
                                           @Parameter(description = "Update a task by given id") @PathVariable("taskId") Long taskId) {
    ...
    @Operation(summary = "Get a task",
            description = "Get a task and its details by given id",
            tags = {"Tasks", "Get"})
    @ApiResponses({
            @ApiResponse(responseCode = "200", content = {@Content(schema = @Schema(implementation = TasksController.class), mediaType = "application/json")}),
            @ApiResponse(responseCode = "404", content = {@Content(schema = @Schema())}),
